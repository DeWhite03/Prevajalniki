		...
		try {
			...
			case LPAR:
				...
				((Expr7Context)_localctx).RPAR = match(RPAR);
				 ((Expr7Context)_localctx).ast =  ((Expr7Context)_localctx).expr.ast; 
				}
				break;
			case SIZEOF:
				{
	...
	public final ExprsContext exprs() throws RecognitionException {
		...
		try {
			...
			((ExprsContext)_localctx).exprs2 = exprs2();
			 ((ExprsContext)_localctx).ast =  ((ExprsContext)_localctx).exprs2.ast; _localctx.ast.addLast(((ExprsContext)_localctx).expr.ast); 
			}
		}
		catch (RecognitionException re) {
			_localctx.exception = re;
